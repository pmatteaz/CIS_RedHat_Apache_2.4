...
# Funzione per verificare se la configurazione LimitExcept è corretta
is_correct_limitexcept() {
    local config="$1"
    if echo "$config" | grep -q "<LimitExcept GET POST HEAD>"; then
        return 0
    else
        return 1
    fi
...
modify_directory_section() {
    ...
    local line_buffer=""
    local in_limitexcept=false
    local limitexcept_found=false
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        if [[ $line =~ ^[[:space:]]*\<Directory[[:space:]]+ ]]; then
            ...
            line_buffer="$line"
            limitexcept_found=false
            continue
        fi

        # Se siamo in una sezione Directory
        if [ "$in_directory" = true ]; do
            # Se troviamo l'inizio di LimitExcept
            if [[ $line =~ ^[[:space:]]*\<LimitExcept ]]; then
                limitexcept_found=true
                in_limitexcept=true
                # Verifica se la configurazione LimitExcept è corretta
                if ! is_correct_limitexcept "$line"; then
                    # Sostituisci la vecchia configurazione con quella nuova
                    ...
                    changes_made=true
                    echo -e "${YELLOW}Modificata configurazione LimitExcept esistente in $current_directory${NC}"
                    # Salta fino alla fine di LimitExcept
                    while IFS= read -r subline; do
                        if [[ $subline =~ ^[[:space:]]*\</LimitExcept\> ]]; then
                            in_limitexcept=false
                            break
                        fi
                    done
                    continue
                fi
            # Se siamo all'interno di LimitExcept ma non è quella corretta
            elif [ "$in_limitexcept" = true ]; then
                if [[ $line =~ ^[[:space:]]*\</LimitExcept\> ]]; then
                    in_limitexcept=false
                fi
                continue
            # Se troviamo la fine della sezione Directory
            elif [[ $line =~ ^[[:space:]]*\</Directory\> ]]; then
                in_directory=false
                # Se non abbiamo trovato LimitExcept, lo aggiungiamo
                if [ "$limitexcept_found" = false ]; then
                    echo "$line_buffer" >> "$temp_file"
                    echo "    <LimitExcept GET POST HEAD>" >> "$temp_file"
                    echo "        Require all denied" >> "$temp_file"
                    echo "    </LimitExcept>" >> "$temp_file"
