...
check_group_write() {
    ...
    while IFS= read -r -d '' file; do
        ...
        perms=$(stat -c '%A' "$file")
        if [[ ${perms:5:3} =~ w ]]; then
            echo -e "${RED}✗ Trovato file con permessi di scrittura del gruppo: $file (${perms:5:3})${NC}"
...
if [ ${#wrong_permissions[@]} -gt 0 ]; then
    ...
    if [[ "$risposta" =~ ^[Ss]$ ]]; then
        ...
        for item in "${wrong_permissions[@]}"; do
            ...
            if [ -d "$item" ]; then
                current_perms=$(stat -c "%A" "$file" | cut -c5-7)
                if [ $current_perms =~ w ]; then
                    # Rimuovi w
                    chmod g-w,g+x "$item"
                fi
            else
            ...
            new_perms=current_perms=$(stat -c "%A" "$file" | cut -c5-7)
            if [ ! $new_perms =~ w ]; then 
